      ...
      <div className="container px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h3 className="text-xl font-bold border-b pb-2">Nuestros Productos y Servicios</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {PRODUCTS.map(product => (
                <motion.div 
                  key={product.id}
                  whileHover={{ scale: 1.02 }}
                  className="border rounded-xl p-4 flex flex-col justify-between bg-gray-50 hover:border-primary transition-colors"
                >
                  <div>
                    <span className="text-xs font-bold text-primary uppercase tracking-wider">{product.category}</span>
                    <h4 className="font-bold mt-1">{product.name}</h4>
